...
const data = fs.readFileSync(`${__dirname}/dev-data.json`, 'uft-8');
const dataObj = JSON.parse(dta)
  ...
  //request has a property call url which gives the current
    ...
    if(pathName === '/' || pathName === '/overview') {
     ...
     fs.readFile(`${__dirname}/dev-data/data.json`, 'utf-8', (err, data) => {
          //The data return from the callback are in json so we need to convert it to js object
          const productData = JSON.parse(data)
          console.log(productData)

          //Sending back the data to the client
          //If we want to do that we have to tell the browswer that we want to send json to the client so we have to specify it in the header
