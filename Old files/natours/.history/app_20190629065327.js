...
app.get('/api/v1/tours/:id/:x?', (req, res) => {
     ...
     //'/api/v1/tours/:id' means for example if i visit this route '/api/v1/tours/1, the id now the id has become the object holding a value called 1 so to access that value you will use req.params.id

     //You can also specify as many parameters and you can make some optional by using a question mark  (?) '/api/v1/tours/:id/:x?'
